...
### Variante équitable avec priorité aux rédacteurs

Un risque classique est la **famine des rédacteurs** : si des lecteurs arrivent sans arrêt, un rédacteur peut attendre très longtemps. Une solution consiste à bloquer les nouveaux lecteurs dès qu'un rédacteur est en attente.
...
\begin{tikzpicture}[
  >=Latex,
  every node/.style={font=\small},
  state/.style={draw, rounded corners=5pt, minimum width=2.5cm, minimum height=0.8cm, align=center},
...
Cette variante évite qu'un flux continu de lecteurs bloque les rédacteurs. En contrepartie, dès qu'un rédacteur attend, les nouveaux lecteurs patientent même si la ressource est encore en lecture.

### Comparaison avec le thread pool

| Patron | Ressource partagée | Threads qui attendent | Condition d'attente |
|:---|:---|:---|:---|
| Thread pool | Queue de tâches | Workers | Queue vide |
| Producteur-consommateur | Tampon borné | Producteurs et consommateurs | Tampon plein ou vide |
| Lecteurs-rédacteurs | Donnée partagée | Lecteurs ou rédacteurs | Écriture active, lecteurs actifs, ou rédacteur en attente |

Le point commun est toujours le même : on protège l'état partagé avec un mutex, puis on exprime les conditions d'entrée avec des variables de condition ou un verrou spécialisé (`std::shared_mutex`). Le patron change surtout la règle d'accès à la ressource.
